...
def train_and_test_PPOd(
    ...
    monitor = False, evaluation = False
):
    """
    Trains and tests a PPO agent on the provided train/test splits.
    ...
    if monitor:
        ...
        monitor_filename = f"{log_dir}/PPOd_{forecast_type}_monitor.csv"
    ...
    if load_existing_model and model_load_path is not None:
        print(f"Loading existing PPO model from {model_load_path}")
        ...
        print(f"Training PPO using {forecast_type} forecasts and sell_price_ratio={sell_price_ratio}")
        model = PPO(**ppo_params)
        model.learn(total_timesteps=total_timesteps, callback=ProgressBarCallback())
        if model_save_path is not None:
            model.save(model_save_path)

    print(f"Testing PPO using {forecast_type} forecasts and sell price ratio: {sell_price_ratio}")
